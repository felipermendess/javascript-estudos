//History - É possível acessarmos o histórico de acesso do browser em uma sessão específica através do window.history. O objeto history possui diferentes métodos e propriedades.
// window.history;
// window.history.back(); // vai para a anterior
// window.history.forward(); // vai para a próxima

//PushState - A parte interessante de manipularmos o history é que podemos modificar o histórico e adicionar um novo item. window.history.pushState(obj, title, url).
...
//PopState - O evento popstate pode ser adicionado ao objeto window. Assim podemos executar uma função toda vez que o usuário clicar no botão de voltar ou próximo.
// window.addEventListener('popstate', () => {
//     fetchPage(window.location.pathname);
// }); //só ativa quando o próprio dev ou user modificar o pushState
...
// window.history.pushState(null,null,'sobre.html'); //obj,tittle,url
